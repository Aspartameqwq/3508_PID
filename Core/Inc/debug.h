...
/** @brief 【已废弃】制动反向阶段固定迭代次数。
 *
 *   旧版制动时长固定为此值 × 10ms。现已改为与制动入口速度成正比的动态时长
 *   （见下方 debug_brake_duration_per_ms），此宏不再在制动逻辑中使用。
 *   保留定义仅为兼容旧版引用。 */
#define BRAKE_HS_COUNT 10U
 ...
 *
 *   判断对象：ChassisToMotors() 解算出的电机目标转速 motor_speeds[i]。
 *   |目标转速| < 阈值 → 认为"摇杆已回中"→ 进入制动流程。
 *   默认 0.3 rad/s。设 0 则完全禁用制动。
 *
 *   Ozone 调参指南：
 *   - 摇杆回中后制动迟迟不触发（车滑很远才停）→ 调大（如 0.5~1.0）
 *   - 轻推摇杆正常行驶中被误判为回中、触发制动 → 调小（如 0.1~0.2）
 *   - 典型值范围 0.1~1.0 rad/s */
 ...
 *
 *   段 0 反向制动的持续时间 = 摇杆回中瞬间的底盘实际平移速度 × 此系数。
 *   速度越快 → 制动越久，速度越慢 → 制动越短，实现与惯性匹配的制动强度。
 *
 *   时间换算：1 次迭代 = CONTROL_PERIOD_MS (10ms)。
 *     例：入口速度 1.5 m/s，系数=25 → 37 次 ≈ 370ms
 *     例：入口速度 0.3 m/s，系数=25 →  7 次 ≈  70ms
 *
 *   默认 25.0（次 / m/s）。设 0 则跳过段 0 直接进入段 1（零电流）。
 *
 *   Ozone 调参指南：
 *   - 制动太猛、车停下后反向弹跳 → 调小（如 15~20）
 *   - 制动太弱、车滑很远才停 → 调大（如 30~40）
 *   - 与 debug_brake_reverse_speed_ms 配合：线速度大 + 时长长 = 强力制动 */
extern volatile float debug_brake_duration_per_ms;

/** @brief 【段 0】制动最大持续迭代次数（安全上限）。
 *
 *   防止极端高速下制动时间过长（如 4 m/s × 25 = 100 次 = 1 秒）。
 *   动态计算的制动次数超过此值时强制截断。
 *
 *   默认 80 次（800ms）。设为 0 则不设上限（慎用）。 */
extern volatile float debug_brake_max_duration;
...
/** @brief CCW 偏航修正对角死区角（°）。
 *
 *   遥控器推向纯对角线方向时（左前/左后/右前/右后），车轮本身会产生
 *   旋转分量，此时叠加 CCW 修正会导致过旋转。此参数定义速度矢量方向
 *   与一三象限或二四象限角平分线（y=x 或 y=-x）之间的夹角阈值：
 *     若 |速度方向角 - 最近对角线| < 此阈值 → 禁用 CCW 修正
 *     若 |速度方向角 - 最近对角线| ≥ 此阈值 → 正常应用 CCW 修正
 *
 *   例如：推正前（0°=y轴）→ 与最近对角线 45° 相差 45° > 15° → CCW 生效 ✓
 *        推右前（约 45°）→ 与对角线 45° 相差 ≈0 < 15° → CCW 禁用 ✓
 *
 *   默认 15°。设 0 则禁用死区（所有方向均应用 CCW 修正）。
 *   Ozone 调参：死区太小 → 对角线仍有偏航 → 调大；死区太大 → 非对角线也不修正 → 调小。 */
extern volatile float debug_kinematics_ccw_dead_angle_deg;
